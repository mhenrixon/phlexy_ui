...
describe DaisyUI::RadialProgress do
  ...
  context "when size and thickness are not provided" do
    let(:component) do
      Class.new(Phlex::HTML) do
        def view_template(&)
          render DaisyUI::RadialProgress.new(value: 50) do
            "50%"
          end
        end
      end
  ...
  context "when size is provided but thickness is not" do
    let(:component) do
      Class.new(Phlex::HTML) do
        def view_template(&)
          render DaisyUI::RadialProgress.new(value: 50, size: "6rem") do
            "50%"
          end
        end
      end
  ...
  context "when thickness is provided but size is not" do
    let(:component) do
      Class.new(Phlex::HTML) do
        def view_template(&)
          render DaisyUI::RadialProgress.new(value: 50, thickness: "10%") do
            "50%"
          end
        end
      end
  ...
  describe "rendering a full radial progress" do
    let(:component) do
      Class.new(Phlex::HTML) do
        def view_template(&)
          render DaisyUI::RadialProgress.new(
            :neutral,
            value: 50,
            size: "6rem",
            thickness: "10%",
